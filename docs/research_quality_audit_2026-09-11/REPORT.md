...
- catalog: `F:\codex\wqb_alpha_factory\.wqb_state\platform_field_catalog_20260910`；fetched_at: `2026-09-10T07:10:21.249470+00:00`；status: `COMPLETE_WITH_SUPPLEMENTS`
...
- current proposal diversity: `{"proposal_count": 100, "expression": {"unique_count": 100, "diversity_ratio": 1.0}, "structure": {"unique_family_count": 19, "dominant_family": "compounding_pressure", "dominant_share": 0.1}, "semantic": {"known_mechanism_count": 7, "unknown_mechanism_count": 37, "dominant_mechanism": "fundamental:change:slow_moving", "dominant_share": 0.31746031746031744}, "field_concepts": {"unique_known_count": 5, "unknown_count": 37}, "datasets": {"unique_count": 6}, "lineages": {"unique_independent_count": 15, "unknown_count": 0, "dominant_lineage_share": 0.1}, "warnings": [], "dominant_mechanism": "fundamental:change:slow_moving", "dominant_mechanism_share": 0.31746031746031744, "dominant_structure": "compounding_pressure", "dominant_structure_share": 0.1, "layers": {"optimization": {"count": 0, "mechanism_count": 0, "lineage_count": 0, "field_concept_count": 0}, "exploration": {"count": 100, "mechanism_count": 7, "lineage_count": 15, "field_concept_count": 5}}}`
- current mechanism keys: **8**；UNKNOWN: **37**
- dry-run batch count: **100**
- dry-run priority distribution: `{'NORMAL': 92, 'LOW': 8}`

### Mechanism review candidates

- `market_price:change:signed+market_price:level:signed:same_economic_concept`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['relative_correlation', 'relative_covariance']；relationships=['same_economic_concept']
- `fundamental:change:slow_moving`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'distributional_change', 'extreme_location', 'extreme_low', 'group_centered_level', 'persistent_level']；relationships=['None']
- `sentiment:level:event_driven`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'distribution_regime', 'event_trigger', 'group_centered_level', 'persistent_level', 'robust_cross_section', 'vector_change', 'vector_persistence']；relationships=['None']
- `option_relative:level:unknown`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'distributional_change', 'extreme_location', 'group_centered_level', 'persistent_level', 'robust_cross_section']；relationships=['None']
...
- `sentiment:ratio:event_driven`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'distributional_change', 'event_trigger', 'robust_cross_section']；relationships=['None']
...
| NORMAL | pcr_oi_120 | `robust_cross_section` | 字段 pcr_oi_120 被识别为 option_relative，测量为 ratio，频率为 unknown，符号语义为 bounded，行为为 bounded；put-call/skew 字段表达期权分布的相对位置，适合离散或相对关系检验。该机制仍需用独 | 字段 pcr_oi_120 的 robust_cross_section 结构是否提供可复现的增量信号？ |
...
| NORMAL | implied_volatility_call_10 | `robust_cross_section` | 字段 implied_volatility_call_10 被识别为 volatility，测量为 ratio，频率为 unknown，符号语义为 nonnegative_level，行为为 signed；波动率是风险暴露或状态变量，适合风险调整、regime | 字段 implied_volatility_call_10 的 robust_cross_section 结构是否提供可复现的增量信号？ |
| NORMAL | anl4_fs_detail_estimates_advanced_af_nd_epsr_std | `distributional_change` | 字段 anl4_fs_detail_estimates_advanced_af_nd_epsr_std 被识别为 analyst_dispersion，测量为 dispersion，频率为 annual，符号语义为 nonnegative_dispersion | 字段 anl4_fs_detail_estimates_advanced_af_nd_epsr_std 的 distributional_change 结构是否提供可复现的增量信号？ |
...
- mechanism identity: 100 historical proposals、8 个已见机制 key、37 个 UNKNOWN；100/100 expression unique、19 个 structure families、15 个 lineages。6 个 cluster 是 false-merge review candidates，当前没有独立证据确认 false split/merge。
