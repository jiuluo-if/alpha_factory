...
| Alpha template catalog | `wqb_agent.alpha_templates` | public synthetic schema/example + explicit private resolver/registry/numeric/operator audit；真实模板只在 local private catalog，`alpha_factory.py` 与 `candidate.py` 只能消费 |
