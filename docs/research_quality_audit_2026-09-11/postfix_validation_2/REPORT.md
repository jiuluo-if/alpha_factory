...
- catalog: `F:\codex\wqb_alpha_factory\.wqb_state\platform_field_catalog_20260910`；fetched_at: `2026-09-10T21:08:29.779767+00:00`；status: `COMPLETE_WITH_SUPPLEMENTS`
...
- current proposal diversity: `{"proposal_count": 100, "expression": {"unique_count": 100, "diversity_ratio": 1.0}, "structure": {"unique_family_count": 19, "dominant_family": "distribution_regime", "dominant_share": 0.18}, "semantic": {"known_mechanism_count": 11, "unknown_mechanism_count": 10, "dominant_mechanism": "market_price:change:sparse", "dominant_share": 0.12222222222222222}, "field_concepts": {"unique_known_count": 6, "unknown_count": 10}, "datasets": {"unique_count": 6}, "lineages": {"unique_independent_count": 20, "unknown_count": 0, "dominant_lineage_share": 0.11}, "warnings": [], "dominant_mechanism": "market_price:change:sparse", "dominant_mechanism_share": 0.12222222222222222, "dominant_structure": "distribution_regime", "dominant_structure_share": 0.18, "layers": {"optimization": {"count": 0, "mechanism_count": 0, "lineage_count": 0, "field_concept_count": 0}, "exploration": {"count": 100, "mechanism_count": 11, "lineage_count": 20, "field_concept_count": 6}}}`
- current mechanism keys: **12**；UNKNOWN: **10**
- dry-run batch count: **100**
- dry-run priority distribution: `{'NORMAL': 92, 'LOW': 8}`

### Mechanism review candidates

- `earnings:probability:slow_moving`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['distribution_regime', 'distributional_change', 'robust_cross_section']；relationships=['None']
- `fundamental:change:slow_moving`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['distribution_regime', 'distributional_change']；relationships=['None']
- `liquidity:ratio:signed`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'downside_risk', 'risk_adjusted_reversal', 'robust_cross_section']；relationships=['None']
- `market_price:change:sparse`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['adaptive_scale_change', 'distribution_regime', 'extreme_low', 'group_centered_level', 'group_relative_change', 'group_relative_extreme', 'innovation_surprise', 'last_update_surprise', 'risk_adjusted_reversal', 'trend_residual', 'turnover_control']；relationships=['None']
- `market_price:count:signed`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'downside_risk', 'risk_adjusted_reversal', 'robust_cross_section']；relationships=['None']
- `market_price:level:signed`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'distribution_regime', 'group_centered_level', 'persistent_level', 'robust_cross_section']；relationships=['None']
- `sentiment:level:event_driven`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['adaptive_scale_change', 'compounding_pressure', 'distribution_regime', 'group_centered_level', 'persistent_level']；relationships=['None']
- `sentiment:ratio:event_driven`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'distributional_change', 'robust_cross_section']；relationships=['None']
- `volatility:dispersion:signed`：同一 key 下出现多个 template/relationship，需人工判断 false merge；families=['accumulated_change', 'adaptive_scale_change', 'compounding_pressure', 'delayed_confirmation', 'distribution_regime', 'distributional_change', 'downside_risk', 'extreme_location', 'risk_adjusted_reversal']；relationships=['None']
...
- mechanism identity: 100 historical proposals、12 个已见机制 key、10 个 UNKNOWN；100/100 expression unique、19 个 structure families、15 个 lineages。6 个 cluster 是 false-merge review candidates，当前没有独立证据确认 false split/merge。
